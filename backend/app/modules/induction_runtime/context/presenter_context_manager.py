...
class PresenterContextManager:
    ...
    def load_presenter_profile(self, db: DBSession) -> Dict[str, Any]:
        ...
        if not config:
            logger.warning("PresenterContextManager | No OrganizationConfig found. Falling back to default profile.")
            self.profile = {
                "company_name": "KONE",
        ...
        logger.info(f"PresenterContextManager | Loaded profile: {self.profile['ai_trainer_name']} at {self.profile['company_name']}")
        return self.profile
